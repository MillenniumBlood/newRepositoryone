...
{
    m_portNumber = pNumber;
    m_blocking = 1;

    try
    {
		if ( (m_cltSockId=socket(AF_INET,SOCK_STREAM,0)) == -1)
        {
			#ifdef WINDOWS_XP
				int errorCode;
				string errorMsg = "";
	...
	*/
    ...
    m_stutNetAddr.sin_port = htons(m_portNumber);
}
...
void MySocket::setDebug(int debugToggle)
{
	try 
	{
		if ( setsockopt(m_cltSockId,SOL_SOCKET,SO_DEBUG,(char *)&debugToggle,sizeof(debugToggle)) == -1 )
		{
			#ifdef WINDOWS_XP
				int errorCode;
...
void MySocket::setReuseAddr(int reuseToggle)
{
	try 
	{
		if ( setsockopt(m_cltSockId,SOL_SOCKET,SO_REUSEADDR,(char *)&reuseToggle,sizeof(reuseToggle)) == -1 )
		{
			#ifdef WINDOWS_XP
				int errorCode;
...
void MySocket::setKeepAlive(int aliveToggle)
{
	try 
	{
		if ( setsockopt(m_cltSockId,SOL_SOCKET,SO_KEEPALIVE,(char *)&aliveToggle,sizeof(aliveToggle)) == -1 )
		{
			#ifdef WINDOWS_XP
				int errorCode;
...
void MySocket::setLingerSeconds(int seconds)
{
	...
	try 
	{
		if ( setsockopt(m_cltSockId,SOL_SOCKET,SO_LINGER,(char *)&lingerOption,sizeof(struct linger)) == -1 )
		{
			#ifdef WINDOWS_XP
				int errorCode;
...
void MySocket::setSendBufSize(int sendBufSize)
{
	try 
	{
		if ( setsockopt(m_cltSockId,SOL_SOCKET,SO_SNDBUF,(char *)&sendBufSize,sizeof(sendBufSize)) == -1 )
		{
			#ifdef WINDOWS_XP
				int errorCode;
...
void MySocket::setReceiveBufSize(int receiveBufSize)
{
	try 
	{
		if ( setsockopt(m_cltSockId,SOL_SOCKET,SO_RCVBUF,(char *)&receiveBufSize,sizeof(receiveBufSize)) == -1 )
		{
			#ifdef WINDOWS_XP
				int errorCode;
...
void MySocket::setSocketBlocking(int blockingToggle)
{
	...
	{
		#ifdef WINDOWS_XP
			if (ioctlsocket(m_cltSockId,FIONBIO,(unsigned long *)&m_blocking) == -1)
			{
				int errorCode;
		...
		#ifdef UNIX
			if (ioctl(m_cltSockId,FIONBIO,(char *)&m_blocking) == -1)
			{
				MyException unixSocketOptionException(0,"unix: error getting host by name");
				throw unixSocketOptionException;
			}
...
int MySocket::getDebug()
{
	...
	try 
	{
		if ( getsockopt(m_cltSockId,SOL_SOCKET,SO_DEBUG,(char *)&myOption,&myOptionLen) == -1 )
		{
			#ifdef WINDOWS_XP
				int errorCode;
...
int MySocket::getReuseAddr()
{
	...
	try 
	{
		if ( getsockopt(m_cltSockId,SOL_SOCKET,SO_REUSEADDR,(char *)&myOption,&myOptionLen) == -1 )
		{
			#ifdef WINDOWS_XP
				int errorCode;
...
int MySocket::getKeepAlive()
{
	...
	try 
	{
		if ( getsockopt(m_cltSockId,SOL_SOCKET,SO_KEEPALIVE,(char *)&myOption,&myOptionLen) == -1 )
		{
			#ifdef WINDOWS_XP
				int errorCode;
...
int MySocket::getLingerSeconds()
{
	...
	try 
	{
		if ( getsockopt(m_cltSockId,SOL_SOCKET,SO_LINGER,(char *)&lingerOption,&myOptionLen) == -1 )
		{
			#ifdef WINDOWS_XP
				int errorCode;
...
int MySocket::getSendBufSize()
{
	...
	try 
	{
		if ( getsockopt(m_cltSockId,SOL_SOCKET,SO_SNDBUF,(char *)&sendBuf,&myOptionLen) == -1 )
		{
			#ifdef WINDOWS_XP
				int errorCode;
...
int MySocket::getReceiveBufSize()
{
	...
	try 
	{
		if ( getsockopt(m_cltSockId,SOL_SOCKET,SO_RCVBUF,(char *)&rcvBuf,&myOptionLen) == -1 )
		{
			#ifdef WINDOWS_XP
				int errorCode;
...
void MyTcpSocket::bindSocket()
{
	try
	{
		if (bind(m_cltSockId,(struct sockaddr *)&m_stutNetAddr,sizeof(struct sockaddr_in))==-1)
		{
			#ifdef WINDOWS_XP
				int errorCode = 0;
...
{ 
	...
	*/
	...
	MyHostInfo serverInfo(serverNameOrAddr,hType);//获得hostent
	
    // Store the IP address and socket port number	
	struct sockaddr_in serverAddress;	//‘sockaddr_in’、‘serverAddress’
    serverAddress.sin_family = AF_INET;
    serverAddress.sin_addr.s_addr = inet_addr(serverInfo.getHostIPAddress());
    serverAddress.sin_port = htons(m_portNumber); //与clientAddress中一样的m_portNumber

    // Connect to the given address
	try 
	{
		if (connect(m_cltSockId,(struct sockaddr *)&serverAddress,sizeof(serverAddress)) == -1)
		{
			#ifdef WINDOWS_XP
				int errorCode = 0;
...
MyTcpSocket* MyTcpSocket::acceptClient(string& clientHost)
{
	...
	try 
	{
		if ((newSocket = accept(m_cltSockId, (struct sockaddr *)&clientAddress,&clientAddressLen)) == -1)
		{
			#ifdef WINDOWS_XP
				int errorCode = 0;
...
void MyTcpSocket::listenToClient(int totalNumPorts)
{
	try 
	{
		if (listen(m_cltSockId,totalNumPorts) == -1)
		{
			#ifdef WINDOWS_XP
				int errorCode = 0;
...
int MyTcpSocket::sendMessage(string& message)
{
	...
	*/
	...
	char msgLength[MSG_HEADER_LEN+1];
    sprintf_s(msgLength,MSG_HEADER_LEN+1,"%6d",message.size());
	string sendMsg = string(msgLength);
    sendMsg += message;
	...
	try 
	{
		if (numBytes = send(m_cltSockId,sendMsg.c_str(),sendMsg.size(),0) == -1)
		{
			#ifdef WINDOWS_XP
				int errorCode = 0;
				string errorMsg = "error calling send():\n";
...
int MyTcpSocket::XpRecieveMessage(string& message)
{
	...
	char msgLength[MSG_HEADER_LEN+1]; //package header(len)
	...
	try
	{
		...
		{
			numBytes = recv(m_cltSockId,charMsg+received,1,0);
			if (numBytes == -1)
			{
				int errorCode = 0;
				string errorMsg = "error calling recv():\n";
...
int MyTcpSocket::recieveMessage(string& message)
{
	...
	#ifdef WINDOWS_XP
		return XpRecieveMessage(message);	//此处就return了
	#endif
	...
	try
	{
		numBytes = recv(m_cltSockId,msgLength,MSG_HEADER_LEN,0);
        if (numBytes == -1)
        {
	...
	try
	{
		numBytes = recv(m_cltSockId,(char*)(message.c_str()),atoi(msgLength),0);
        if (numBytes == -1)
        {
			MyException unixSocketRecvException(0,"unix: error calling recv()");
			throw unixSocketRecvException;
