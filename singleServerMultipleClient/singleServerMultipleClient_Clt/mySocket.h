...
protected:
	...
	*/
	MySocket() {}
	void setSocketId(int socketFd) { m_cltSockId = socketFd; }

protected:

    int m_portNumber;        // Socket port number    
    int m_cltSockId;          // Socket file descriptor

    int m_blocking;          // Blocking flag
    int m_bindFlag;          // Binding flag

    struct sockaddr_in m_stutNetAddr;    // Address of the client that sent data
...
public:
    ...
    int  getSocketBlocking() { return m_blocking; }
	int  getLingerSeconds();
    bool getLingerOnOff();
	
    // returns the socket file descriptor
    int getSocketId() { return m_cltSockId; }
...
public:
	...
	*/
	MyTcpSocket() {};	//默认构造函数
	MyTcpSocket(int portId) : MySocket(portId) { };	//带个端口的构造函数
	~MyTcpSocket();
